...
const BgColorSidePanel = ({ canvasRef, setBgColor }) => {
  return (
    ...
    <div className="color-pallet flex flex-wrap justify-evenly gap-2 my-[50px] xs:my-[30px] vsm:mb-40 gsm:gap-2 gsm:pb-[5rem] gsm:pt-[4rem] gsm:mb-8 mx-auto w-full xs:w-[40%]">
      <input
        type="color"
        name="color"
        id="color"
        ...
        className={` cursor-pointer  w-[3rem] h-[3rem] xs:w-[2rem] xs:h-[2rem] rounded-[0.4rem] border-[0.2px] border-black bg-gradient-to-r from-red-700 via-yellow-600 to-green-600 `}
      />
  ...
  return (
    <>
      <div
        ...
        className={`m-0 cursor-pointer w-[3rem] h-[3rem] xs:w-[2rem] xs:h-[2rem] rounded-[0.4rem] border-[0.2px] border-black shadow-lg shadow-black  dark:border-2 dark:border-black hover:scale-110 hover:border-2`}
        onClick={() => {
          setBgColor(color);
